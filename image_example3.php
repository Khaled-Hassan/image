...
<html>
    ...
    <body>
        <form id="uploadPhoto" action="image_example3.php" method="post" enctype="multipart/form-data">
        ...
        if (isset($_FILES['photo']) && in_array($_FILES['photo']['type'], $allowedtypes)){
            ...
            $image8 = $storePath . $image->addText('Khaled', FALSE, 60, 9, ['R' => 255, 'G' => 0, 'B' => 0])[0];
            $image9 = $storePath . $image->watermark_image('logo.png', 1, 0.7, FALSE)[0];
            ...
            $img->quality = 80;
            $img->fontName = 'source/font/M Unicode Susan.ttf';
            $img->newImageName = [$image->newImageName[0]];
            $img->getImageData($storePath . $image->newImageName[0]); // $newImageName is name the uploaded image after save in server 
            
            $image18 = $img->resizeSameRatio(800, 600)[0];
            $image19 = $img->effectBrightness(50, TRUE)[0];
            $image20 = $img->effectWhiteBlack(TRUE)[0];
            $image21 = $img->addText('Khaled', FALSE, 60, 2, ['R' => 0, 'G' => 0, 'B' => 255], TRUE)[0];
            $image22 = $storePath . $img->watermark_image('logo.png', 9, 0.8, FALSE, TRUE)[0];
            ?>
        ...
        <br><br><br>
        <div id="new22"><h1>New instance resize with same ratio, change brightness, make white & black, add text and add watermark<br>Note: delete old image after every step</h1> <br><img src="<?php echo $image22; ?>"></div>
        <?php }?>
